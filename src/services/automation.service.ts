...
export class AutomationService {
  async runWarrantyCheck(): Promise<AutomationRunResult> {
    const now = new Date();
    ...
    const candidates: NotificationCandidate[] = equipments.map((equipment) => ({
  ...
  async runIncidentEscalationCheck(): Promise<AutomationRunResult> {
    ...
    const candidates: NotificationCandidate[] = incidents.map((incident) => {
  ...
  async runMaintenanceOverdueCheck(): Promise<AutomationRunResult> {
    ...
    const candidates: NotificationCandidate[] = maintenances.map((maintenance) => {
  ...
  private async createNotificationsIfNotExists(
    type: NotificationType,
    candidates: NotificationCandidate[]
  ): Promise<number> {
    if (candidates.length === 0) return 0;

    const createdForDispatch = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    externalNotificationService.notifyAsync(
      createdForDispatch.map((item) => ({
        type,
        campusId: item.campusId,
        entityId: item.entityId,
        message: item.message,
    ...
    logger.info(
      { type, processed: candidates.length, created: createdForDispatch.length },
