 ...
 */
export function requireAdminWithCampus(req, res, next) {
    if (!req.user) {
        res.status(401).json({ error: 'Unauthorized: User not authenticated' });
        return;
    }
 ...
 */
export function checkCampusAccess(getResourceCampusId, getResourceUserId) {
    return (req, res, next) => {
        if (!req.user) {
            res.status(401).json({ error: 'Unauthorized: User not authenticated' });
            return;
        ...
        const access = checkResourceAccess({ ...req.user, campus_id: req.campusScope?.campus_id }, resourceCampusId, resourceUserId);
        if (!access.allowed) {
 ...
 */
export function requireOwnershipOrElevated(getResourceUserId) {
    return (req, res, next) => {
        if (!req.user) {
            res.status(401).json({ error: 'Unauthorized: User not authenticated' });
            return;
