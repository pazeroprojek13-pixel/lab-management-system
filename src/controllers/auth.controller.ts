...
export class AuthController {
  ...
  async register(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const currentRole = currentUser.role as ExtendedRole;

      // Validate role assignment based on current user's role
      const standardRoles: Role[] = [Role.ADMIN, Role.LAB_ASSISTANT, Role.LECTURER, Role.STUDENT];
      const elevatedRoles: Role[] = [Role.SUPER_ADMIN, Role.DEVELOPER];
      ...
      const requestRole = role as Role;
      let userRole: Role;
  ...
  async updateUser(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const targetUserRole = targetUser.role as ExtendedRole;
      const newRole = role as ExtendedRole;
