...
export class EventController {
    ...
    async findById(req, res) {
        try {
            ...
            // Check campus access
            const access = checkResourceAccess(req.user, event.campusId || undefined, event.createdById);
            if (!access.allowed) {
    ...
    async update(req, res) {
        try {
            ...
            // Check campus access
            const access = checkResourceAccess(req.user, existingEvent.campusId || undefined, existingEvent.createdById);
            if (!access.allowed) {
                res.status(403).json({ error: access.reason });
                return;
            }
