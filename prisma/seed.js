...
async function main() {
    ...
    if (!existingLabs) {
        ...
        const labs = await prisma.lab.createMany({
            data: [
                { name: 'Computer Lab 1', code: 'LAB-001', capacity: 30, location: 'Building A, Floor 1', status: 'ACTIVE', campusId: campus.id },
                { name: 'Computer Lab 2', code: 'LAB-002', capacity: 25, location: 'Building A, Floor 2', status: 'ACTIVE', campusId: campus.id },
                { name: 'Networking Lab', code: 'LAB-003', capacity: 20, location: 'Building B, Floor 1', status: 'ACTIVE', campusId: campus.id },
            ],
        });
